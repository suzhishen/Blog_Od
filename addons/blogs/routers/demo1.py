...
async def search(env: Annotated[Environment, Depends(odoo_env)]):
    ...
    # raise HTTPException(status.HTTP_400_BAD_REQUEST, "测试异常返回")
    logging.info('213大大打发')
    count1 = env["res.users"].search_count([])
    record1 = env["res.users"].search([])
    rows = env["blogs"].sudo().search_count([])
    record = env["blogs"].sudo().search([])
    return {
        'code': 200,
        'rows': rows,
        'message': 'success, 列表查询',
...
async def _search(self, domain=[], page=1, limit=40, sort=''):
    ...
    if offset < 0:
        offset = 0
    return self.env['blogs'].sudo().search(domain, offset=offset, limit=limit, order=sort)


async def _create(form_data):
    res = request.env['blogs'].sudo().create([form_data])
    return res
